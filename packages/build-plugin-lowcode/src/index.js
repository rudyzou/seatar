...
function getUsedComponentViews(rootDir, targetDir = 'lowcode', components) {
  let viewPaths = glob.sync(
    path.resolve(rootDir, `${targetDir}/**/view.@(js|ts|jsx|tsx)`)
  )
  if (viewPaths && viewPaths.length) {
    viewPaths = viewPaths.map((item) => {
      return item.slice(
        path.resolve(rootDir, targetDir).length + 1,
        item.lastIndexOf('view') - 1
      )
    })
  }
  return components
...
function getUsedComponentMetas(
  rootDir,
  lowcodeDir = 'lowcode',
  metaFilename,
  components
...
async function registerSetter(context, setterMap) {
  return Promise.all(
    Object.values(setterMap).map((setter) => installModule(context, setter))
  )
}
...
function confirmRenderPlatforms(rootDir, platforms) {
  const result = platforms.filter((item) => {
    ...
    const res = glob.sync(
      path.resolve(rootDir, `${viewPath}/**/view.@(js|ts|jsx|tsx)`)
    )
    return res.length
  })
...
async function build(options, pluginOptions, execCompile) {
  ...
  renderViewPaths.forEach((item) => {
    renderViewPathMap[path.basename(item).replace(path.extname(item), '')] =
      item
...
async function start(options, pluginOptions) {
  ...
  const { viewPath, metaPathMap, renderViewPathMap, platforms } =
    await debounceBuild(options, pluginOptions, false)
  const devViewUrls = !disableStyleLoader
    ? ['/view.js']
    : ['/view.js', '/view.css']
...
async function initLowCodeSchema(
  ...
  const lowcodeDirExists = await fse.existsSync(
    path.resolve(rootDir, lowcodeDir)
  )
  if (lowcodeDirExists) {
  ...
  filteredComponents.forEach((item) => {
    ...
    if (
      !fse.existsSync(
        path.resolve(
          rootDir,
          `${lowcodeDir}/${componentNameFolder}/meta${metaDevSubfix}.${
            metaFormat || 'ts'
          }`
        )
      ) &&
      !fse.existsSync(
        path.resolve(
          rootDir,
          `${lowcodeDir}/${componentNameFolder}/meta${metaDevSubfix}.js`
        )
      )
    ) {
...
async function bundleMetaV2(options, pluginOptions, execCompile, metaType) {
  ...
  if (components && !Array.isArray(components)) {
    console.error(
      '[@alifd/build-plugin-lowcode] components must be Array<ComponentName: string>'
    )
    components = null
  }
  ...
  const componentsImportStr = fullbackComponents
    .map((component) => {
      ...
      if (
        ...
        fullbackComponents.includes(component)
      ) {
  ...
  const metaPath = generateEntry({
    ...
    params: {
      ...
      basicLibraryVersion: JSON.stringify(
        customBasicLibraryVersion || BASIC_LIBRARY_VERSION
      )
    }
  })
  if (!execCompile || getAllTask().includes(`lowcode-meta-${metaType}`))
    return metaPath
...
async function bundleEditorView(
  ...
  if (singleComponent && fse.existsSync(lowcodeViewPath)) {
    ...
    const _componentViews =
      getUsedComponentViews(rootDir, lowcodeDir, components) || []
    ...
    componentViewsImportStr = _componentViews
      .map((component) => {
        ...
        const viewJsPath = path.resolve(
          rootDir,
          `${lowcodeDir}/${componentNameFolder}/view`
        )
  ...
  if (
    ...
    editUrls ||
    umdUrls ||
    getAllTask().includes('lowcode-editor-view')
  )
    return viewPath
...
async function bundleRenderView(options, pluginOptions, platform, execCompile) {
  ...
  if (
    ...
    editUrls ||
    umdUrls ||
    getAllTask().includes(`render-view-${platform}`)
  )
    return viewPath
  ...
  onGetWebpackConfig(`render-view-${platform}`, (config) => {
    ...
    config.output.library(library).libraryTarget('umd')
    config.output.path(
      path.resolve(rootDir, `${buildTarget}/${lowcodeDir}/render/${platform}`)
    )
    config.externals({ ...COMMON_EXTERNALS, ...externals })
    if (baseLibrary === 'rax') {
      const scssRule = config.module.rule('scss')
  ...
  onHook('after.build.compile', () => {
    ['dev', 'daily', 'prod'].forEach((item) => {
      ...
      const targetPath = path.resolve(
        rootDir,
        `${buildTarget}/${lowcodeDir}/${filename}`
      )
      ...
      fse.outputFileSync(
        targetPath,
        JSON.stringify(require(originPath), null, 2)
      )
    })
...
async function bundleComponentMeta(
  ...
  const { registerTask, getAllTask, onGetWebpackConfig, context, onHook } =
    options
  ...
  const usedDevComponents = getUsedComponentMetas(
    rootDir,
    `meta${metaDevSubfix}`,
    components
  )
  ...
  usedDevComponents.forEach((component, idx) => {
    (function (comp, index) {
      ...
      if (!execCompile || getAllTask().includes(taskName))
      ...
      onGetWebpackConfig(taskName, (config) => {
        ...
        config.output.path(
          path.resolve(rootDir, `${buildTarget}/${lowcodeDir}`)
        )
        config.externals({ ...COMMON_EXTERNALS, ...externals })
        useStyleLoader(config)
      })
